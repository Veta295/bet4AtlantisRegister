...
const beta = new MessageEmbed().setDescription(`**${kullanici} kişisi başarılı bir şekilde kayıtsıza atıldı.**`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0xd85555').setTimestamp()
message.channel.send(beta).then(x => x.delete({timeout: 10000}));
